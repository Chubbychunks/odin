...
fix_parameters <- function(x) {
  ...
  x$gamma02 = c(prog_rate, prog_rate)
  x$gamma03 = c(prog_rate, prog_rate)
  ...
  x$gamma01 <- c(x$gamma01, x$gamma01)
  x$gamma04 <- c(x$gamma04, x$gamma04)
  x$omega <- c(x$omega, 1-(x$omega))
...
lhs_parameters <- function(n, sample = NULL) {
  ...
  samples_list <- lapply(samples_list, fix_parameters)
