...
generate_parameters <- function(...) {
  ...
  if (is.null(names(given)) || !all(nzchar(names(given)))) {
    stop("All arguments must be named")
  }
  extra <- setdiff(names(given), names(defaults))
  if (length(extra) > 0L) {
    stop("Unknown arguments: ", extra)
  }
  modifyList(defaults, given)
}

generate_parameters()
generate_parameters(theta=1)
...
tmp <- lapply(seq_len(nrow(pp)), function(i) f(pp[i,], parameters, v, gen, time))
names(tmp[[1]])

x <- tmp[[1]]
x[grep("^[SI]", names(tmp[[1]]))]
xx <- x[grep("^[SI]", names(tmp[[1]]))]

rowSums(do.call(cbind, xx))
